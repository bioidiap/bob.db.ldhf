...
def generate_temp_files(dev_files, legends):

  distances  = ['1m','60m','100m','150m']
  temp_files     = []
  temp_filenames = []
  temp_legends   = []
  ...
  for i  in range(len(dev_files)):
    ...
    lines = open(dev_f).readlines()

    #Creating the temporary files
    for d in distances:
      temp_filename = "./"+legends[i] + "_" + d
      temp_filenames.append(temp_filename)
      temp_files.append(open(temp_filename,"w"))
      temp_legends.append(legends[i] + "-" + d)
    
    #reading the original score files
    for l in lines:
    
      for j in range(len(distances)):

        if(l.find(distances[j]) >= 0):
          temp_files[j].write(l)
          
     #closing the files
    #for j in range(len(distances)):
    temp_files = []
  
  return [temp_filenames,temp_legends]
...
def command_line_arguments(command_line_parameters):
  ...
  [args.dev_files,args.legends] = generate_temp_files(args.dev_files,args.legends)
...
def main(command_line_parameters=None):
  ...
  if args.criterion or args.roc or args.det or args.cllr or args.mindcf:
    ...
    if args.mindcf:
      logger.info("Computing minDCF on the development ")
      for i in range(len(scores_dev)):
        # compute threshold on development set
    ...
    if args.cllr:
      logger.info("Computing Cllr and minCllr on the development ")
      for i in range(len(scores_dev)):
